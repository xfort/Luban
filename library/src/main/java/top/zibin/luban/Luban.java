...
public class Luban implements Handler.Callback {
    ...
    private Bitmap.CompressFormat outFormat;
    private int quality;
    ...
    private File getImageCacheFile(Context context, String suffix) {
        ...
        String cacheBuilder = mTargetDir + "/" + System.currentTimeMillis() + (int) (Math.random
                () * 1000) + (TextUtils.isEmpty(suffix) ? ".jpg" : suffix);
    ...
    private File get(InputStreamProvider input, Context context) throws IOException {
        return new Engine(input, getImageCacheFile(context, Checker.SINGLE.extSuffix(input)),
                focusAlpha).setTargetSize(targetWidth, targetHeight).setOutFormat(outFormat)
                .setQuality(quality).compress();
    }
    ...
    private File compress(Context context, InputStreamProvider path) throws IOException {
        ...
        if (mCompressionPredicate != null) {
            if (mCompressionPredicate.apply(path.getPath()) && Checker.SINGLE.needCompress
                    (mLeastCompressSize, path.getPath())) {
                result = new Engine(path, outFile, focusAlpha).setQuality(quality).setOutFormat
                        (outFormat).setTargetSize(targetWidth, targetHeight).compress();
            } else {
                result = new File(path.getPath());
            }
        } else {
            result = Checker.SINGLE.needCompress(mLeastCompressSize, path.getPath()) ? new Engine
                    (path, outFile, focusAlpha).setTargetSize(targetWidth, targetHeight)
                    .setQuality(quality).setOutFormat(outFormat).compress() : new File(path
                    .getPath());
        }
    ...
    public static class Builder {
        ...
        public <T> Builder load(List<T> list) {
            for (T src : list) {
                if (src instanceof String) {
                    ...
                    throw new IllegalArgumentException("Incoming data type exception, it must be " +
                            "" + "" + "" + "" + "" + "" + "String, File, Uri or Bitmap");
                }
            }
            return this;
        }
         ...
         *                   <p> false - don't keep alpha channel, it might have a black
         *                   background.</p>
         */
        ...
        public Builder setOutFormat(Bitmap.CompressFormat outformat) {
